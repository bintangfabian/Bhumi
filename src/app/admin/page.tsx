...
export default function AdminPage() {
  const [packs, setPacks] = useState<AdminPack[]>(ADMIN_PACKS);
  const [selPack, setSelPack] = useState("a1");
  const [openStage, setOpenStage] = useState<string | null>("s3");

  const sel = packs.find((p) => p.id === selPack) ?? packs[0];
  const totalDays = sel.stages.reduce((n, s) => n + (parseInt(s.days, 10) || 0), 0);
  const estDays = parseInt(sel.days, 10) || 0;
  const dayNote =
    totalDays === estDays
  ...
  function editStage(sid: string, fn: (s: AdminStage) => AdminStage) {
    editPack((p) => ({
      ...p,
  ...
  return (
    <Container className="py-10 lg:py-14">
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-line pb-6">
        ...
        <Button onClick={addPack} variant="primary" size="sm">
          + Paket baru
        </Button>
      </div>
      ...
      <div className="mt-8 grid items-start gap-8 lg:grid-cols-[240px_1fr]">
        ...
        <div className="space-y-8">
          ...
          <section className="border border-line bg-surface">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line px-5 py-3.5">
              <span className="text-[12.5px] text-ink-3">
                Foto produk dan isi paket dikelola di tab Media.
              </span>
          ...
          <section className="border border-line bg-surface">
                  ...
                  return (
                    <div key={st.id} className="border-b border-line">
                      <div className="flex items-center gap-3 py-3">
                        <button
                          onClick={() =>
                            setOpenStage((cur) => (cur === st.id ? null : st.id))
                          }
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line px-5 py-3.5">
              <span className="text-[12.5px] text-ink-3">
                Perubahan tersimpan otomatis sebagai draf.
              </span>
